...
- A先发送ARP协议,询问所有的网络中的主机，谁的IP地址是目标B主机IP地址(目标MAC ff::ff::ff交换机转发,交换机记录A-1)，所有主机收到ARP
- 目标B的主机回应ARP响应，告诉A我是目标IP地址，并给到自己的MAC地址,通过交换机回复A主机(交换机记录B-2),并将A的MAC地址存入自己的ARP缓存表中
- 这一次，A的ARP缓存表缓存了B主机的MAC地址,然后再次发送数据,这次就可以直接交付给B主机了..见步骤1

2. 不同的网段

- 首先检查自己的ARP缓存表,有没有缓存路由器的MAC地址,如果有,直接发送,不再进行下一步...
- 我们事先已经在电脑中设置了我们的网关 -- 也就是我们的路由器的IP地址
- A先发送ARP协议，询问所有的网络中的主机，谁的IP地址是网关(目标MAC ff::ff::ff交换机转发,交换机记录A-1)
- 路由器收到后,记录A的IP地址对应的MAC地址到自己的ARP高速缓存表中(路由器记录A-MAC)
- 路由器响应ARP,告诉源IP地址，我是目标IP地址，并给到自己端口的MAC地址,通过交换机发给A(交换机记录R1-2) 
- 到达A后A的ARP缓存表缓存了路由器的MAC地址(A记录路由MAC),然后再发送数据,这次可以直接发送给路由器了..见步骤1
...
- 如果只有端口的信息,说明是同一个网络了已经(直连)
- 路由器首先查找ARP缓存表，看下有没有目标IP地址的MAC地址，有直接交换机转发
- 没有的话,丢弃该帧,路由器发ARP请求,通过交换机(ff::ff::ff广播)广播给所有的主机(交换机记录R-1)
- 目标IP地址的主机B收到后,首先缓存路由器IP地址和MAC地址，然后通过交换机(B-2))转发给路由器
- 路由器缓存了目标IP地址的MAC地址
- 下次数据再来的时候,从步骤四直接发送给目标主机

第二种情况:下一跳路由--路由器

- 路由器到达端口后因为找不到目标IP地址对应的MAC地址直接丢弃该帧
- 路由器检查自己的ARP缓存表,看下有没有下一跳的MAC地址,有的话直接发送出去
- 路由器发送ARP请求,得到下一跳的IP地址(路由器)的MAC地址,并缓存起来
- 转发给同一网段中的另一个路由器,另一个路由器ARP缓存来访路由器的MAC地址
- 回到第一种情况,或者继续第二种情况
...
- 滑动窗口,服务器告知自己的窗口大小，防止发送的客户端发送太快，服务器接收不过来

可以看得出来，所有的流量控制，都是服务器主动控制的，服务器根据拥塞窗口调整发送的包的次数,再根据滑动窗口来调整是否需要停止一下直到接收方有足够的缓冲区接收
...
- 拥塞窗口 一个包的大小大概是1460字节,发送放的流量控制,防止发送方发的太快
