...
目标的MAC地址可以是个广播地址或者是个组播地址，这里，需要进一步的确认应用的场景分别是什么，广播的应用场景感觉应该是为了得到具体的Mac地址才进行的泛洪(待确认?)
...
### TCP 发送方发送数据过快的处理方法(客户端发送的总是很快,但服务器接收的速度总是很慢)
...
- 序号和确认号,A 在发送的数据包中增加一个序号(seq),同时 b 要在 ACK 包增加一个确认号(ack)
