...
- 如表中找不到相应的端口则把数据包广播到除自己以外的所有端口上(这个叫洪泛,盲目转发)，当目的机器对源机器回应时，交换机又可以学习一目的MAC地址与哪个端口对应，在下次传送数据时就不再需要对所有端口进行广播了
- 不断的循环这个过程，对于全网的MAC地址信息都可以学习到，二层交换机就是这样建立和维护它自己的地址表

> 交换机记录信息  = MAC地址 与 端口的映射，交换机只负责转发,无论发给它的是谁，又或者它即将发给谁


## 4. 网络层

`路由器`主要工作在网络层,也是网络层的主要设备
