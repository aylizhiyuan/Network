...
- A发送ARP协议,询问所有的网络中的主机，谁的IP地址是目标IP地址(目标MAC ff::ff::ff交换机转发,交换机记录A-1)，所有主机收到ARP
- 目标B的主机回应ARP响应，告诉A我是目标IP地址，并给到自己的MAC地址,通过交换机回复源IP地址(交换机记录B-2)
- 这一次，A带着自己的IP地址 + 目标IP地址 + 源MAC地址 + 目标MAC地址直接将数据通过交换机传递给目标主机

2. 不同的网段

- 我们事先已经在电脑中设置了我们的网关 -- 也就是我们的路由器的IP地址
- A发送ARP协议，询问所有的网络中的主机，谁的IP地址是网关(目标MAC ff::ff::ff交换机转发,交换机记录A-1)
- 路由器R1响应ARP,告诉源IP地址，我是目标IP地址，并给到自己的MAC地址,通过交换机发给A(交换机记录R1-2) 
- 我们带着IP地址 + 目标IP地址(真实的，不是网关的) + 源MAC地址 + 目标MAC地址(路由器) 通过交换机发送给路由器

3. 但是假设一开始A就知道B的MAC地址，而不需要发送ARP请求的话，则交换机需要通过盲目转发的方式来记录MAC地址对应的端口，这种情况可以理解为有一个集线器连接了一个交换机的情况...
...
- 然后找自己的ARP缓存中是否缓存了下一跳的MAC地址，如果有直接发过去
- 如果没有的话，发送ARP请求下一跳的MAC地址，并记录到ARP缓存表中(如果是路由器的话,路由器1 --> ARP ---> 路由器2;如果是主机的话,路由器 --> ARP --> 主机;如果是一个交换机的话,交换机 ----> ARP ----> 所有的主机)
- 最后重新封装成帧发送出去
